...
then
	...
	do
		...
		then
			...
			if ! test "$song" = ""
			then
				#artist=`echo "$line" | cut -d'"' -f1 | sed -r -e's/^# *//;s/ [^0-9a-zA-Z]*$//;s/\[\[[^]]*?\|([^]]*?)]]/\1/g;s/]//g;s/\[//g;s/]//g'`
				artist=`echo "$line" | sed -r -e 's/\xE2\x80\x93.*?$//;s/\[\[[^]]*?\|([^]]*?)]]/\1/g;s/]//g;s/\[//g;s/^#//;s/^ *//;s/ *$//'`
				#songname=`echo "$line" | cut -d'"' -f2 | sed -r -e 's/^ *//;s/ *$//;s/\[\[[^]]*?\|([^]]*?)\]\]/\1/;s#/#-#g;s/\[//g;s/]//g'`
			...
			album=`echo "$line" | egrep 'Triple J Hottest 100, [0-9]{4}\|[0-9]{4}' | egrep 'style' | egrep -o 'Triple J Hottest 100, [0-9]{4}'`
			if ! test "$album" = ""
			then
				cnt=1
				mkdir "$album"
